...
function buildWasmGraph() {
  ...
  try {
    ...
    console.log(
      `[bv-graph] WASM graph: ${store.wasmGraph.nodeCount()} nodes, ${store.wasmGraph.edgeCount()} edges`,
    );
  } catch (e) {
...
export async function initGraph(containerId, options = {}) {
  ...
  store.graph = ForceGraph()(store.container)
    ...
    // Forces
    .d3Force("charge", d3.forceManyBody().strength(store.config.chargeStrength).distanceMax(300))
    .d3Force(
      "link",
      d3
        .forceLink()
    ...
    .d3Force("x", d3.forceX().strength(store.config.centerStrength))
    .d3Force("y", d3.forceY().strength(store.config.centerStrength))
    .d3Force(
      "collision",
      d3.forceCollide().radius((node) => getNodeSize(node) + 5),
    )
  ...
  if (store.wasmReady) {
    ...
    if (!layout?.metrics) {
      ...
      if (layout.cycles && store.wasmGraph) {
        const cyclesAsIndices = layout.cycles
          .map((cycle) =>
...
function prepareGraphData(layout = null) {
  ...
  nodes = nodes.map((issue) => {
    ...
    return {
      ...
      pagerank:
        pre?.pagerank ?? (idx !== undefined && metrics.pagerank ? metrics.pagerank[idx] : 0),
      betweenness:
        pre?.betweenness ??
        (idx !== undefined && metrics.betweenness ? metrics.betweenness[idx] : 0),
      ...
      dependentCount:
        pre?.outDegree ?? dependencies.filter((d) => d.depends_on_id === issue.id).length,
...
function drawNode(node, ctx, globalScale) {
  ...
  // Border
  ctx.strokeStyle = isSelected ? THEME.accent.purple : isHovered ? THEME.fg : THEME.bgSecondary;
...
function getLinkColor(link) {
  ...
  if (
    store.connectedNodes.size > 0 &&
    store.connectedNodes.has(srcId) &&
    store.connectedNodes.has(tgtId)
  ) {
...
function getLinkWidth(link, globalScale) {
  ...
  if (store.heatmapMode && sourceNode && targetNode) {
    ...
    const avgBetweenness =
      (srcBetweenness + tgtBetweenness) / 2 / (store.maxMetrics.betweenness || 1);
...
function animateWhatIfCascade(sourceNode, result) {
  ...
  const unblockedIds = (result.unblocked_ids || [])
    .map((idx) => {
    ...
    setTimeout(
      () => {
        ...
        store.dependencies.forEach((dep) => {
          ...
          if (
            blocker &&
            (blocker.status === "closed" || dep.issue_id === sourceNode.id) &&
...
export function animateCriticalPath(animate = true) {
  ...
  // Convert indices to node IDs
  criticalPathState.path = longestPath.nodes
    .map((idx) => store.wasmGraph.nodeId(idx))
    .filter(Boolean);
...
function showCriticalPathSummary() {
  ...
  showToast(`Critical path: ${path.length} nodes, from ${sourceId} to ${sinkId}`, "info");
...
export function initCycleNavigator() {
  ...
  cycleNavigatorState.cycles = store.metrics.cycles.cycles
    .map((cycle) => cycle.map((idx) => store.wasmGraph?.nodeId(idx)).filter(Boolean))
    .filter((cycle) => cycle.length > 0);
...
export function prevCycle() {
  if (!cycleNavigatorState.active) return null;
  const prevIndex =
    (cycleNavigatorState.currentIndex - 1 + cycleNavigatorState.cycles.length) %
    cycleNavigatorState.cycles.length;
...
export function getCycleNavigatorState() {
  return {
    ...
    currentPath: formatCyclePath(
      cycleNavigatorState.cycles[cycleNavigatorState.currentIndex] || [],
    ),
  };
}
...
function applyRadialLayout() {
  ...
  store.graph
    ...
    .d3Force(
      "radial",
      d3.forceRadial((node) => (node.criticalDepth || 0) * 80, center.x, center.y).strength(0.5),
    )
    .d3ReheatSimulation();
}
...
function applyLabelGalaxyLayout() {
  ...
  store.graph
    ...
    .d3Force(
      "collision",
      d3.forceCollide().radius((node) => getNodeSize(node) + 8),
    )
    .d3ReheatSimulation();
...
function applyPreset(presetName) {
  ...
  if (!preset) {
    console.warn(
      `[Graph] Unknown preset: ${presetName}. Available: ${Object.keys(LAYOUT_PRESETS).join(", ")}`,
    );
    return false;
  }
  ...
  if (customForces.gridSize && customForces.gridStrength) {
    ...
    store.graph
      ...
      .d3Force(
        "y",
        d3
          .forceY((node) => (node.criticalDepth || 0) * customForces.depthSpacing)
...
function navigateByPriority(direction) {
  ...
  const currentIdx = currentId ? sortedNodes.findIndex((n) => n.id === currentId) : -1;
...
function navigateByKCore(direction) {
  ...
  const currentKCoreNodes = graphData.nodes.filter(
    (n) => (n.kcore ?? 0) === kCores[currentKCoreIdx],
  );
  ...
  if (direction > 0) {
    ...
    if (currentIdx >= 0 && currentIdx < currentKCoreNodes.length - 1) {
      ...
      const higherKCoreNodes = graphData.nodes.filter(
        (n) => (n.kcore ?? 0) === kCores[currentKCoreIdx + 1],
      );
    ...
    if (currentIdx > 0) {
      ...
      const lowerKCoreNodes = graphData.nodes.filter(
        (n) => (n.kcore ?? 0) === kCores[currentKCoreIdx - 1],
      );
      targetNode = lowerKCoreNodes[lowerKCoreNodes.length - 1];
    } else {
      // Wrap to last node in highest k-core
      const highestKCoreNodes = graphData.nodes.filter(
        (n) => (n.kcore ?? 0) === kCores[kCores.length - 1],
      );
...
function showTooltip(node) {
  ...
  if (node.betweenness !== undefined && isFinite(node.betweenness)) {
    extendedMetrics.push(
      `<span title="Bottleneck centrality">Between: ${safeMetric(node.betweenness * 100, 1, "%")}</span>`,
    );
  }
  ...
  if (node.eigenvector !== undefined && isFinite(node.eigenvector)) {
    extendedMetrics.push(
      `<span title="Influence centrality">Eigen: ${safeMetric(node.eigenvector * 100, 1, "%")}</span>`,
    );
  }
  if (node.slack !== undefined && isFinite(node.slack)) {
    const slackColor =
      node.slack < 3 ? THEME.priority.p0 : node.slack < 7 ? THEME.priority.p1 : THEME.fgMuted;
    extendedMetrics.push(
      `<span style="color: ${slackColor}" title="Schedule flexibility">Slack: ${formatSlack(node.slack)}</span>`,
    );
  }
...
export function transformHistoryToTimeline(robotHistory) {
  ...
  const commits = [...commitMap.values()].sort((a, b) => new Date(a.date) - new Date(b.date));
