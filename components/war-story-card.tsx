...
export default function WarStoryCard({ story }: { story: WarStory }) {
  ...
  return (
    <FrankenContainer
      ...
      withPulse={true}
      accentColor="#ef4444"
      className="group h-full glass-modern transition-all duration-500 hover:bg-white/[0.03] hover:border-red-500/30 hover:-translate-y-1 border-white/5"
    >
      <div className="flex h-full flex-col p-8 md:p-10 text-left">
        ...
        <FrankenGlitch trigger="hover" intensity="low">
          <motion.h3
            className="text-2xl font-black text-white mb-2 transition-colors tracking-tight"
        ...
        <div className="mt-auto relative group/debug">
          ...
          <div className="relative rounded-xl bg-black/40 border border-white/5 p-6">
            ...
            <div className="text-xs leading-relaxed text-slate-500 font-mono group-hover:text-slate-400 transition-colors">
              <Streamdown
                content={story.technicalDetails}
                className="text-xs font-mono font-normal text-slate-500 leading-relaxed space-y-2"
              />
            </div>
          </div>
        </div>
        ...
        <div className="mt-8 pt-6 border-t border-white/5 flex items-center justify-between">
          ...
          <span className="text-[10px] font-bold text-red-500/80 uppercase tracking-tighter">
            {story.impact}
          </span>
        </div>
      </div>
