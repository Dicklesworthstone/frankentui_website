...
import { ArrowRight, Eye, Heart, MessageSquare, Repeat2, Zap } from "lucide-react";
    ...
    <div className="flex flex-col p-6 md:p-7">
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <div className="flex flex-col text-left">
          <p className="text-sm font-black text-slate-200 leading-tight tracking-tight">
            {tweet.author}
          </p>
        ...
        <span className="ml-auto text-[10px] font-bold text-slate-600 font-mono bg-white/5 px-2 py-1 rounded-md">
          {tweet.date}
        </span>
      </div>
...
function GlassFrankenCard({
  ...
  return (
    <div
      className={`franken-tweet-card relative rounded-2xl overflow-hidden border border-white/10 bg-black/40 backdrop-blur-xl shadow-2xl hover:border-green-500/30 transition-all duration-700 hover:shadow-green-500/10 group ${className ?? ""}`}
    >
      {/* Top stitch + bolts */}
        ...
        <div data-theme="dark" className="p-1">
          ...
          */}
          <ErrorBoundary fallback={<FallbackContent tweet={tweet} />}>
            <ReactTweet id={tweet.tweetId!} fallback={<FallbackContent tweet={tweet} />} />
          </ErrorBoundary>
        </div>
      </GlassFrankenCard>
    </motion.div>
