    ...
    >
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-6 lg:px-8">
        <FrankenContainer
          ...
          withPulse={true}
          accentColor="#22c55e"
          className="glass-modern p-12 md:p-16 border-green-500/10"
        >
              ...
              <div className="space-y-4">
                <Link href="/" className="flex items-center gap-3 group w-fit">
                  <FrankenGlitch trigger="hover" intensity="low">
                    <div className="h-8 w-8 rounded-lg bg-green-500 flex items-center justify-center font-black text-black text-xs transition-transform group-hover:scale-110">
                      F
                    </div>
                  </FrankenGlitch>
                  <span className="text-xl font-black text-white uppercase tracking-tighter group-hover:text-green-400 transition-colors">
                    {siteConfig.name}
                  </span>
                </Link>
                <p className="text-slate-400 font-medium leading-relaxed max-w-xs text-left">
                  The monster technical kernel for Rust. Built for architectural purity and
              ...
              <div className="flex flex-col gap-3">
                <div className="flex items-center gap-3 text-[10px] font-black uppercase tracking-[0.3em] text-green-500/60">
                  <motion.div
                    ...
                    transition={
                      prefersReducedMotion ? { duration: 0 } : { duration: 1.5, repeat: Infinity }
                    }
                ...
                <nav className="flex flex-col gap-4">
                  {navItems.slice(0, 4).map((item) => (
                    <NavLink
                      key={item.href}
                      href={item.href}
                      className="text-sm font-bold text-slate-500 hover:text-green-400 transition-colors uppercase tracking-widest hover:translate-x-1 duration-200"
                    >
                      {item.label}
                    </NavLink>
                  ))}
