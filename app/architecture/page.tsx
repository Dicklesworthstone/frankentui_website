...
import { Activity, ArrowRight, Binary, Cpu, Lock, Play, Shield, Terminal, Zap } from "lucide-react";
...
import { algorithms, optimizations, performanceSLAs } from "@/lib/content";
...
function DecisionCard({
  ...
  return (
    <FrankenContainer
      ...
      className="group glass-modern h-full p-6 md:p-10 transition-all hover:bg-white/[0.03] border-white/5 hover:border-white/10"
    >
          ...
          <div className="flex flex-col items-start">
            ...
            <FrankenGlitch trigger="random" intensity="low">
              <h1 className="text-6xl md:text-8xl font-black tracking-tight text-white mb-8 text-left">
                Inside the <br />
                <span className="text-animate-green">Machine.</span>
              </h1>
            </FrankenGlitch>
            ...
            >
              A deep dive into the 16-byte cell architecture, Bayesian rendering strategies, and the
              zero-unsafe kernel that powers FrankenTUI.
            </motion.p>
          </div>
        </div>
            ...
            return (
              ...
              >
                <FrankenContainer
                  withBolts={false}
                  withPulse={true}
                  className="glass-modern p-6 md:p-8 text-center h-full flex flex-col items-center hover:bg-green-500/5 hover:border-green-500/30 transition-all overflow-hidden border-green-500/10"
                >
                  <motion.div
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
              ...
              <div className="p-6 md:p-8">
                ...
                <div className="grid grid-cols-[4fr_4fr_4fr_2fr_2fr] border-x border-b border-green-500/30 rounded-b-lg overflow-hidden">
                  {[
                    ...
                    >
                      <span className="text-[11px] md:text-xs font-black text-green-400 font-mono">
                        {field.name}
                      </span>
                      <span className="text-[9px] md:text-[10px] text-green-500/50 font-mono mt-1">
                        {field.size}
                      </span>
                      {field.detail && (
                        <span className="text-[9px] md:text-[10px] text-green-500/30 font-mono mt-0.5">
                          {field.detail}
                        </span>
                      )}
                    </div>
          ...
          <div className="grid gap-6">
            {[
              {
                title: "Cache Optimal",
                desc: "4 cells fit perfectly in a 64-byte L1 cache line.",
                icon: Zap,
              },
              {
                title: "SIMD Ready",
                desc: "Single 128-bit instruction for full cell equality.",
                icon: Cpu,
              },
              {
                title: "Zero Leak",
                desc: "RAII-backed grapheme cleanup prevents memory growth.",
                icon: Shield,
              },
            ].map((feature, i) => (
              <motion.div
              ...
              >
                ...
                <div>
                  <h4 className="text-white font-black uppercase tracking-widest text-sm mb-1 group-hover:text-green-400 transition-colors">
                    {feature.title}
                  </h4>
          ...
          {[
            {
              ...
              desc: "All stdout goes through TerminalWriter. Single serialization point eliminates race conditions.",
              eyebrow: "Concurrency_Safety",
            },
            {
              icon: Shield,
              title: "RAII Cleanup",
              desc: "State restored on drop—even on panic. Your terminal never stays broken.",
              eyebrow: "Kernel_Integrity",
            },
            {
              icon: Activity,
              title: "Deterministic",
              desc: "Identical Model state + terminal size = bit-identical ANSI output. Always.",
              eyebrow: "Correctness_Protocol",
            },
            {
              icon: Zap,
              title: "Zero Unsafe",
              desc: "#![forbid(unsafe_code)] across all core rendering and layout crates.",
              eyebrow: "Memory_Security",
            },
          ].map((item, i) => (
            <motion.div
              key={item.title}
          ...
          <table className="w-full text-left text-sm border-collapse min-w-[600px]">
            <thead>
              <tr className="border-b border-white/10 bg-white/5">
                <th className="p-6 font-black text-white uppercase tracking-widest text-[10px]">
                  Metric
                </th>
                <th className="p-6 font-black text-green-400 uppercase tracking-widest text-[10px]">
                  Target
                </th>
                <th className="p-6 font-black text-yellow-400 uppercase tracking-widest text-[10px]">
                  Hard Cap
                </th>
                <th className="p-6 font-black text-slate-400 uppercase tracking-widest text-[10px]">
                  Notes
                </th>
              </tr>
            </thead>
            <tbody className="divide-y divide-white/5">
              {performanceSLAs.map((sla) => (
                <tr key={sla.metric} className="group hover:bg-white/[0.02] transition-colors">
                  <td className="p-6 font-black text-white text-base group-hover:text-green-400 transition-colors">
                    {sla.metric}
                  </td>
                  ...
                  <td className="p-6 text-sm text-slate-500 leading-relaxed max-w-xs">
                    {sla.notes}
                  </td>
                </tr>
              ))}
          ...
          <FrankenContainer className="glass-modern bg-green-500/5 p-12 md:p-20 text-center overflow-hidden border-green-500/20">
            <div className="relative z-10 max-w-2xl mx-auto">
              ...
              <p className="text-lg text-slate-400 font-medium mb-12">
                Add FrankenTUI to your project and start building interfaces with correctness
